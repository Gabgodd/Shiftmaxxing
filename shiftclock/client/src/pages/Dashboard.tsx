...
import { RotateCcw, Clock, AlignLeft } from "lucide-react";
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-80 xl:w-96 flex flex-col border-l border-border overflow-hidden shrink-0">
            ...
            <div className="grid grid-cols-3 border-b border-border shrink-0">
              ...
              <KpiCell label="Overtime"  value={`+${totalOvertimeHours.toFixed(1)}h`}     accent={totalOvertimeHours > 0} />
            </div>
...
function ShiftLever({
  ...
  baseHours, overtimeHours, releasedHours,
}: {
  ...
  const onMouseDown = (e: React.MouseEvent, type: "start" | "end" | "move") => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      if (t === "start") {
        const ns = Math.max(0, Math.min(startEnd - 0.5, startVal + delta));
        onLeverChange(shift.id, ns, startEnd);
      } else if (t === "end") {
        const ne = Math.max(startVal + 0.5, Math.min(24, startEnd + delta));
        onLeverChange(shift.id, startVal, ne);
      } else {
        const dur = startEnd - startVal;
        const ns = Math.max(0, Math.min(24 - dur, startVal + delta));
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-1.5">
          ...
          {hasReleased && (
            <span className="text-[9px] px-1.5 py-0.5 rounded-sm font-medium bg-red-500/20 text-red-400">
              -{releasedHours.toFixed(1)}h free
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1">
          <button onClick={() => adjustStart(-0.5)} className="text-[9px] px-1.5 py-0.5 rounded bg-muted hover:bg-accent transition-colors" title="Earlier start">← 30m</button>
          <button onClick={() => adjustStart(0.5)}  className="text-[9px] px-1.5 py-0.5 rounded bg-muted hover:bg-accent transition-colors" title="Later start">30m →</button>
          <span className="text-[10px] font-mono text-muted-foreground mx-1">{formatH(activeStart)}</span>
        </div>
        <span className="text-[9px] text-muted-foreground font-mono tabular-nums">
          {activeDuration.toFixed(1)}h
        </span>
        <div className="flex items-center gap-1">
          <span className="text-[10px] font-mono text-muted-foreground mx-1">{formatH(activeEnd)}</span>
          <button onClick={() => adjustEnd(-0.5)} className="text-[9px] px-1.5 py-0.5 rounded bg-muted hover:bg-accent transition-colors" title="Earlier end">← 30m</button>
          <button onClick={() => adjustEnd(0.5)}  className="text-[9px] px-1.5 py-0.5 rounded bg-muted hover:bg-accent transition-colors" title="Later end">30m →</button>
        </div>
      </div>
    </div>
  );
          ...
          return (
            <div key={agent.id} className="flex items-start gap-2 text-[10px]">
              ...
              <div className="flex-1">
                ...
                <span className="text-muted-foreground ml-1">{activeHours.toFixed(1)}h</span>
                {overtimeHours > 0 && <span className="ml-1" style={{ color: agent.color }}>+{overtimeHours.toFixed(1)}h OT</span>}
                {releasedHours > 0 && <span className="ml-1 text-red-400">{releasedHours.toFixed(1)}h up for grabs</span>}
              </div>
            </div>
          );
        })}
